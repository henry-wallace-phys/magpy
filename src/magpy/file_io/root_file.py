...
class RootFile:
    ...
    def __init__(self, file_name: str) -> None:
        ...
        if self.file is None:
            raise MagpyInvalidObjectError(
                f"File {file_name} could not be opened or is empty."
            )
    ...
    def close(self):
        ...
        logging.warning(
            "File [bold]%s[/] was already closed or not opened.", self.file_name
        )
    ...
    def get_root_object(self, object_name: str) -> Any:
        ...
        try:
            ...
            raise MagpyInvalidObjectError(
                f"Failed to get ROOT object '{object_name}': {e}"
            ) from e
...
class IterableFile(RootFile):
    ...
    def __getitem__(self, key: str) -> Any:
        ...
        if key not in self.file_keys:
            raise MagpyInvalidObjectError(
                f"Key '{key}' not found in file keys: {self.file_keys}"
            )
