...
class SplineFile(RootFile):
    """JAX Spline file handler for the magpy ecosystem."""

    BINNING_HIST_STR = "dev_tmp.0.0;1"

    def __init__(self, file_path: str):
        """Initialize the JAXSplineFile class.

        :param file_path: Path to the ROOT file.
        :type file_path: str
        """
        print(f"Loading JAX spline file: {file_path}")
        super().__init__(file_path)

        print("Extracting splines")
        # We now convert the TGraphs to JAX CubicSpline objects
        spline_array = []
        self._spline_names = []
        ...
        for graph_name in tqdm(self.file.keys(), desc="Systematic"):
            ...
            try:
                x, y = gr.values()
                spline_array.append(Spline(jnp.array(x), jnp.array(y)))
