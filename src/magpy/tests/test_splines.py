...
def test_flat_spline():
    ...
    spline = Spline(torch.tensor([0, 1, 2, 3], dtype=torch.float64), torch.tensor([0, 0, 0, 0], dtype=torch.float64))
...
def test_monolith():
    ...
    flat_example = Spline(torch.tensor([0, 1, 2, 3], dtype=torch.float64), torch.tensor([0, 0, 0, 0], dtype=torch.float64))
    spline = Spline(torch.tensor([0, 1, 2, 3], dtype=torch.float64), torch.tensor([0, 1, 2, 3], dtype=torch.float64))
    ...
    assert torch.isclose(spline_monolith[0], flat_example.spline, rtol=1e-8, atol=1e-8).all()
    assert torch.isclose(spline_monolith[1], spline.spline, rtol=1e-8, atol=1e-8).all()

    x_vals = torch.tensor([10,1.5])
    assert torch.isclose(spline_monolith(x_vals), torch.tensor([1,1.5], dtype=torch.float64), rtol=1e-8, atol=1e-8).all()
