...
class Oscillator:
    ...
    def calc_probability(
        ...
        Ut2sq = (
            osc_params[OscParIndex.S13SQ.value]
            * osc_params[OscParIndex.S12SQ.value]
            * osc_params[OscParIndex.S23SQ.value]
        )
        Um2sq = (1 - osc_params[OscParIndex.S12SQ.value]) * (
            1 - osc_params[OscParIndex.S23SQ.value]
        )
        ...
        Dmsqee = (
            osc_params[OscParIndex.DMSQ31.value]
            - osc_params[OscParIndex.S12SQ.value] * osc_params[OscParIndex.DMSQ21.value]
        )  # Dmsq21

        # calculate A, B, C, See, Tee, and part of Tmm
        A = (
            osc_params[OscParIndex.DMSQ21.value] + osc_params[OscParIndex.DMSQ31.value]
        )  # temporary variable
        See = (
            A
            - osc_params[OscParIndex.DMSQ21.value] * Ue2sq
            - osc_params[OscParIndex.DMSQ31.value] * Ue3sq
        )
        Tmm = (
            osc_params[OscParIndex.DMSQ21.value] * osc_params[OscParIndex.DMSQ31.value]
        ...
        xmat = Amatter / Dmsqee
        tmp = 1 - xmat
        lambda3 = osc_params[OscParIndex.DMSQ31.value] + 0.5 * Dmsqee * (
            xmat
            - 1
            + torch.sqrt(tmp * tmp + 4 * osc_params[OscParIndex.S13SQ.value] * xmat)
        )
        ...
        Smm = (
            A
            - osc_params[OscParIndex.DMSQ21.value] * Um2sq
            - osc_params[OscParIndex.DMSQ31.value] * Um3sq
        )
        ...
        Jmatter = (
            Jmatter
            * osc_params[OscParIndex.DMSQ21.value]
            * osc_params[OscParIndex.DMSQ31.value]
            * (
                osc_params[OscParIndex.DMSQ31.value]
                - osc_params[OscParIndex.DMSQ21.value]
        ...
        out_prob[(osc_in == NuType.E.value) & (osc_out == NuType.E.value)] = Pee[
            (osc_in == NuType.E.value) & (osc_out == NuType.E.value)
        ]
        out_prob[(osc_in == NuType.E.value) & (osc_out == NuType.Mu.value)] = Pem[
            (osc_in == NuType.E.value) & (osc_out == NuType.Mu.value)
        ]
        out_prob[(osc_in == NuType.E.value) & (osc_out == NuType.Tau.value)] = Pet[
            (osc_in == NuType.E.value) & (osc_out == NuType.Tau.value)
        ]
        out_prob[(osc_in == NuType.Mu.value) & (osc_out == NuType.E.value)] = Pme[
            (osc_in == NuType.Mu.value) & (osc_out == NuType.E.value)
        ]
        out_prob[(osc_in == NuType.Mu.value) & (osc_out == NuType.Mu.value)] = Pmm[
            (osc_in == NuType.Mu.value) & (osc_out == NuType.Mu.value)
        ]
        out_prob[(osc_in == NuType.Mu.value) & (osc_out == NuType.Tau.value)] = Pmt[
            (osc_in == NuType.Mu.value) & (osc_out == NuType.Tau.value)
        ]
        out_prob[(osc_in == NuType.Tau.value) & (osc_out == NuType.E.value)] = Pte[
            (osc_in == NuType.Tau.value) & (osc_out == NuType.E.value)
        ]
        out_prob[(osc_in == NuType.Tau.value) & (osc_out == NuType.Mu.value)] = Ptm[
            (osc_in == NuType.Tau.value) & (osc_out == NuType.Mu.value)
        ]
        out_prob[(osc_in == NuType.Tau.value) & (osc_out == NuType.Tau.value)] = Ptt[
            (osc_in == NuType.Tau.value) & (osc_out == NuType.Tau.value)
        ]
