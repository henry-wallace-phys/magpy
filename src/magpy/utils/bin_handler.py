...
class BinHandler:
    ...
    def get_bin_from_int(self, index: torch.Tensor) -> torch.Tensor:
        ...
        return_tensor = torch.zeros(
            (len(index), len(self.bin_edges), 2),
            dtype=torch.float32,
            device=index.device,
        )

        return_tensor[index < len(self._bin_edge_tensor)] = self._bin_edge_tensor[
            index[index < len(self._bin_edge_tensor) - 1]
        ]
        return_tensor[index >= len(self._bin_edge_tensor)] = torch.tensor(
            [-1.0, -1.0], dtype=return_tensor.dtype, device=return_tensor.device
        )
    ...
    def find_bin(self, kinematic: torch.Tensor) -> torch.Tensor:
        ...
        bin_indices = torch.zeros(
            (kinematic.shape), dtype=torch.int, device=kinematic.device
        )

        for i, edge in enumerate(self.bin_edges):
            bin_indices[:, i] = (
                torch.searchsorted(edge[:-1], (kinematic[:, i].contiguous())) - 1
            )
