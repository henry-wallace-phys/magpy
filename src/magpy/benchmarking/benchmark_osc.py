...
def plot_average_time_per_event(times: np.ndarray, scales: np.ndarray, plot_file='osc_average_time_event.png'):

    times_avged = np.zeros(len(scales)*len(times), dtype=np.float64)
    for i in range(len(scales)):
        times_avged[i*len(times):(i+1)*len(times)] = times[:,i]/scales[i]

    print(f"Average time: ([bold]{np.mean(times_avged)*1_000_000}±{np.std(times_avged)*1_000_000} μs[/bold])")

    plt.figure()
    plt.hist(times_avged[np.abs(times_avged - np.mean(times_avged)) < 3*np.std(times_avged)]*1_000_000, bins=50, alpha=0.7)
